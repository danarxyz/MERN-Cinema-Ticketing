...
import { getMovieFilter } from "@/services/global/global.service";
...
export default function CustomerBrowseGenre() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => getMovieFilter(genreId ?? "", filter),
  });

  const selectedGenre = useMemo(() => {
    if (!genreId) return null;
    return genres.find((item) => item._id === genreId);
  }, [genreId, genres]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p className="text-center mx-auto font-semibold text-sm">{`${selectedGenre?.name} Genre`}</p>
      ...
      <section id="New-Movies" className="flex flex-col gap-4 mt-5 px-5">
        ...
        {data?.data?.filterAllMovies?.map((item) => (
