...
import { setStep } from "@/redux/features/tickets/ticketSlice";
...
export default function CustomerTransaction() {
  ...
  const detailPrice = useMemo(() => {
    ...
    const subtotal = (detail?.seat?.length ?? 0) * (movie?.price ?? 0);
    const ppn = 0.11 * subtotal;
  ...
  const handleTransaction = async () => {
    try {
      const parse = transactionSchema.parse({
        ...
        tax: detailPrice.ppn, // Changed from ppn to tax
        movieId: movie?._id,
        seats: detail?.seat ?? [], // Changed from seat to seats
        date: detail?.time ?? "", // Added date field
        ...
        <div className="accordion group flex flex-col w-full rounded-3xl p-5 gap-4 bg-white/10 has-[:checked]:!h-16 transition-all duration-300 overflow-hidden">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col gap-2">
              <p className="font-semibold">{movie?.theater?.[0].name}</p>
              <p className="text-sm text-premiere-grey">
                {movie?.theater?.[0].city}
              </p>
            </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <p>
              {detail?.seat?.length}{" "}
              {detail?.seat?.length === 1 ? "Seat" : "Seats"}
            </p>
          </div>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
      ...
      {!isBalanceEnough && (
        <div className="flex items-center mb-10 justify-between gap-3 rounded-[20px] p-4 bg-premiere-red mx-5 mt-5">
          <p className="font-semibold">
            Saldo E-wallet Anda tidak mencukupi untuk transaksi ini
          </p>
          <Link
            to={"/wallet/topup"}
