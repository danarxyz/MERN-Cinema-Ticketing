...
import { getWalletTransaction } from "@/services/wallet/wallet.service";
              ...
              <div className="flex flex-col gap-[6px]">
                ...
                <div className="flex items-center gap-2">
                  <img
                    ...
                    alt="icon"
                  />
                  <p className="text-sm text-premiere-green">+ {rupiahFormat(item.price)}</p>
                </div>
              </div>
            </div>
            <p className={cn("w-fit rounded-full p-[4px_6px]  font-semibold text-[10px] leading-[15px]", item.status === "failed" ? "bg-premiere-light-red text-premiere-red" : "", item.status === "pending" ? "bg-yellow-100 text-yellow-500" : "", item.status === "success" ? "bg-green-100 text-green-500" : "")}>
              {item.status}
            </p>
          </div>
        ))}
